...
async function tgTextDocument(token:string,chatId:string,text:string,fileName:string,caption:string,mime="text/csv; charset=utf-8"){
  const form=new FormData();form.append("chat_id",chatId);form.append("document",new Blob([text],{type:mime}),fileName);form.append("caption",caption.slice(0,900));
...
function regionalCsv(d:any){
  const cols=["No","Name","Categories","Brand","Operator","Settlement","Settlement method","Settlement distance m","Address","Address quality","Normalized location","Latitude","Longitude","Sources","Source count","Resolution status","Confidence","Wikidata QID"],cell=(v:any)=>{const s=String(v??"");return /[",\n\r;]/.test(s)?'"'+s.replace(/"/g,'""')+'"':s};
...
function spatialSearchText(d:any){
  ...
  const ux=s?.urban_exposure??null;
  if(ux){const g=ux.ghsl??{},o=ux.osm_context??{};lines.push("Urban exposure: "+String(ux.class??"unknown")+" • confidence "+Math.round(Number(ux.confidence??0)*100)+"%","GHSL: pop 1 км ~"+Math.round(Number(g.population_1km??0))+" • built-up "+(g.built_fraction_1km_pct==null?"—":Number(g.built_fraction_1km_pct).toFixed(1)+"%")+" • OSM buildings "+Number(o.buildings??0),"Контекст 2 км: residential "+Number(o.residential_landuse??0)+" • industrial "+Number(o.industrial_landuse??0)+" • forest "+Number(o.forest_landuse??0)+" • agricultural "+Number(o.agricultural_landuse??0),"POI: schools "+Number(o.schools??0)+" • hospitals "+Number(o.hospitals??0)+" • fuel "+Number(o.fuel_stations??0)+" • energy "+Number(o.energy??0)+" • transport "+Number(o.transport??0));if(Array.isArray(ux.reasons)&&ux.reasons.length)lines.push("Почему: "+ux.reasons.slice(0,3).join(" • "));}
...
async function processAdminUpdate(sb:any,token:string,adminId:string,u:any){
  ...
  if(low.startsWith("/objects_nearest")){
    const p=parseSpatialCommand(raw,"nearest");if(!p){await tg(token,"sendMessage",{chat_id:adminId,text:"Использование: /objects_nearest <lat> <lon> <N> <search_km> <объект>\nПример: /objects_nearest 49.5883 34.5514 5 20 АЗС",reply_markup:panelKeyboard});return true;}
    try{const d=await regionalSearchRequest(p);await tg(token,"sendMessage",{chat_id:adminId,text:spatialSearchText(d),reply_markup:panelKeyboard,disable_web_page_preview:true})}catch(e){await tg(token,"sendMessage",{chat_id:adminId,text:regionalErrorText(e),reply_markup:panelKeyboard})}return true;
  }
  if(low.startsWith("/objects_near")){
    const p=parseSpatialCommand(raw,"near");if(!p){await tg(token,"sendMessage",{chat_id:adminId,text:"Использование: /objects_near <lat> <lon> <radius_km> <объект>\nПример: /objects_near 49.5883 34.5514 5 АЗС",reply_markup:panelKeyboard});return true;}
    try{const d=await regionalSearchRequest(p);await tg(token,"sendMessage",{chat_id:adminId,text:spatialSearchText(d),reply_markup:panelKeyboard,disable_web_page_preview:true})}catch(e){await tg(token,"sendMessage",{chat_id:adminId,text:regionalErrorText(e),reply_markup:panelKeyboard})}return true;
  }
  if(low.startsWith("/objects_city")){
    const p=parseSpatialCommand(raw,"city");if(!p){await tg(token,"sendMessage",{chat_id:adminId,text:"Использование: /objects_city <город> | [radius_km] | <объект>\nПример: /objects_city Полтава | 10 | АЗС\nИли: /objects_city Полтава | АЗС",reply_markup:panelKeyboard});return true;}
    try{const d=await regionalSearchRequest(p);await tg(token,"sendMessage",{chat_id:adminId,text:spatialSearchText(d),reply_markup:panelKeyboard,disable_web_page_preview:true})}catch(e){await tg(token,"sendMessage",{chat_id:adminId,text:regionalErrorText(e),reply_markup:panelKeyboard})}return true;
  }
  if(low.startsWith("/objects_polygon")){
    const p=parseSpatialCommand(raw,"polygon");if(!p){await tg(token,"sendMessage",{chat_id:adminId,text:"Использование: /objects_polygon <объект> | lat,lon;lat,lon;lat,lon...\nПример: /objects_polygon АЗС | 49.53,34.45;49.53,34.65;49.65,34.65;49.65,34.45",reply_markup:panelKeyboard});return true;}
    try{const d=await regionalSearchRequest(p);await tg(token,"sendMessage",{chat_id:adminId,text:spatialSearchText(d),reply_markup:panelKeyboard,disable_web_page_preview:true})}catch(e){await tg(token,"sendMessage",{chat_id:adminId,text:regionalErrorText(e),reply_markup:panelKeyboard})}return true;
  }
  if(low.startsWith("/objects_route")){
    const p=parseSpatialCommand(raw,"route");if(!p){await tg(token,"sendMessage",{chat_id:adminId,text:"Использование: /objects_route <объект> | <corridor_km> | lat,lon;lat,lon...\nПример: /objects_route АЗС | 2 | 49.55,34.40;49.62,34.75",reply_markup:panelKeyboard});return true;}
    try{const d=await regionalSearchRequest(p);await tg(token,"sendMessage",{chat_id:adminId,text:spatialSearchText(d),reply_markup:panelKeyboard,disable_web_page_preview:true})}catch(e){await tg(token,"sendMessage",{chat_id:adminId,text:regionalErrorText(e),reply_markup:panelKeyboard})}return true;
  }
  if(low.startsWith("/objects_geojson")){
    const q=raw.split(/\s+/).slice(1).join(" ").trim(),p=parseRegionalArgs(q);
    if(!p){await tg(token,"sendMessage",{chat_id:adminId,text:"Использование: /objects_geojson <область> <объект/категория> [фильтры]\nПример: /objects_geojson Полтавская область АЗС",reply_markup:panelKeyboard});return true}
    try{
      const d=await regionalSearchRequest(p),body=regionalGeoJson(d),name=String(d?.oblast?.code??d?.oblast_code??"region")+"-"+String(d?.category_key??"objects")+".geojson";
      await tgTextDocument(token,adminId,body,name,"GeoWatch GeoJSON • "+String(d?.oblast?.name_uk??d?.oblast_name??q)+" • "+Number(d?.summary?.resolved_objects??0)+" объектов","application/geo+json; charset=utf-8");
    }catch(e){console.error("admin regional geojson failed:",e instanceof Error?e.message:String(e));await tg(token,"sendMessage",{chat_id:adminId,text:regionalErrorText(e),reply_markup:panelKeyboard})}
    return true
  }
