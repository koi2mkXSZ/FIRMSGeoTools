...
function compactCaption(text:string){
 ...
 for(const prefix of ["Источники (","Длительность:","Контекст OSM:","Рядом:","Погода:","Sentinel-2:","Экспозиция GHSL:","Neptun:"]){if((lines.join("\n")+"\n"+footer).length<=930)break;lines=lines.filter(x=>!x.startsWith(prefix))}
