...
function norm(v:any){return String(v??"").normalize("NFKD").toLowerCase().replace(/[\u0300-\u036f]/g,"").replace(/['’\u02bc"]/g,"").replace(/\b(область|області|областю|обл|region|oblast)\b/gu," ").replace(/[^\p{L}\p{N}]+/gu," ").replace(/\s+/g," ").trim()}
...
async function resolveOblast(sb:any,q:any){const {data,error}=await sb.from("oblasts").select("id,code,name_uk,name_en");if(error)throw error;const raw=String(q??"").trim(),n=norm(raw);if(!n)return null;let best:any=null;for(const o of data??[]){const scores=[String(o.code).toLowerCase()===raw.toLowerCase()?1:0,dice(n,o.name_uk),dice(n,o.name_en)];const score=Math.max(...scores);if(!best||score>best.score)best={...o,score}}if(!best||best.score<.48)return null;const {data:g,error:ge}=await sb.rpc("firewatch_oblast_geometry",{p_oblast_id:best.id});if(ge)throw ge;return{...best,...g}}
...
Deno.serve(async(req:Request)=>{
 try{
  ...
  try{overture=await overtureRegion(bbox,geom,spec);if(overture.status!=="active")errors.push("Overture: "+overture.status+(overture.errors?.length?" • "+overture.errors[0]:""))}catch(e){overture={status:"error",features:[],tiles_total:0,tiles_ok:0};errors.push("Overture: "+errText(e))}
