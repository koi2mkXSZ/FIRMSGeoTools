...
function polygonAreaKm2(g:any){const rings=g?.type==="Polygon"?g.coordinates:g?.type==="MultiPolygon"?g.coordinates.flat():[];let total=0;for(const ring of rings??[]){if(!Array.isArray(ring)||ring.length<3)continue;const lat0=ring.reduce((s:any,p:any)=>s+Number(p?.[1]??0),0)/ring.length,cf=Math.cos(lat0*Math.PI/180),pts=ring.map((p:any)=>[Number(p[0])*111.32*cf,Number(p[1])*111.32]);let a=0;for(let i=0,j=pts.length-1;i<pts.length;j=i++)a+=pts[j][0]*pts[i][1]-pts[i][0]*pts[j][1];total+=Math.abs(a)/2}return total}
