...
Deno.serve(async(req)=>{
 ...
 try{
   ...
   if(!user||!pass){
     const monitor={status:"awaiting_credentials",last_check:new Date().toISOString(),last_error:null,source:"EUMETSAT LSA SAF FRP-PIXEL LSA-502",platform:"MSG",instrument:"SEVIRI",cadence_min:15,latest_public_file:latest,latest_public_slot:latestSlot,directory_http_status:dr.status,credentials_required:true,required_secrets:["LSASAF_USERNAME","LSASAF_PASSWORD"],notification_mode:"supporting-observation-only"};
   ...
   const {data:cached}=selected.length?await sb.from("processed_source_products").select("product_id").eq("source_key","EUMETSAT_MSG").in("product_id",selected):{data:[]};
   const done=new Set((cached??[]).map((x:any)=>String(x.product_id)));
   const pending=selected.filter(x=>!done.has(x));
   let totalFetched=0,totalRows=0; const records:any[]=[]; const warnings:string[]=[]; const processed:string[]=[];
   ...
   for(const name of pending){
     ...
     try{
       ...
       totalRows+=n; processed.push(name); f.close();
     }catch(e){warnings.push(`${name}: ${e instanceof Error?e.message:String(e)}`)}
     try{FS.unlink(fn)}catch{}
   }

   if(processed.length)await sb.from("processed_source_products").upsert(processed.map(product_id=>({source_key:"EUMETSAT_MSG",product_id,product_time:parseSlot(product_id)?.toISOString()??null,processed_at:new Date().toISOString()})),{onConflict:"source_key,product_id"});
   ...
   if(ie)throw ie;
   const monitor={status:"active",last_success_run:new Date().toISOString(),last_error:null,source:"EUMETSAT LSA SAF FRP-PIXEL LSA-502",platform:"MSG",instrument:"SEVIRI",cadence_min:15,latest_public_file:latest,latest_public_slot:latestSlot,files_checked:selected.length,files_pending:pending.length,files_cached:selected.length-pending.length,hdf_rows_scanned:totalRows,ukraine_bbox_rows:records.length,ingest:ing,warnings:warnings.slice(-20),credentials_required:false,notification_mode:"supporting-observation-only",match_radius_m:5000};
   await sb.from("system_state").upsert({key:"monitor_eumetsat",value:monitor,updated_at:new Date().toISOString()});
